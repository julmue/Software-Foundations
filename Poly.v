...
0 because it cannot be instantiated.
...
Proof.
    intros X l1 l2 v. induction l1. reflexivity.
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
    match l with
    | [] => ([], [])
    | (x, y)::l' => match split l' with (lx, ly) => (x::lx, y::ly) end
    end.  
...
Lemma snoc_app : forall (X: Type) (l: list X) (x: X),
    snoc l x = l ++ [x].
Proof.
    intros X l x. induction l. reflexivity.
    simpl. rewrite -> IHl. reflexivity.
Qed.

Lemma fmap_app : forall (X Y: Type) (f: X -> Y) (l1: list X) (l2: list X),
    map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof.
    intros X Y f l1 l2. induction l1. reflexivity.
    simpl. rewrite -> IHl1. reflexivity.
Qed.
...
Proof.
    intros X Y f l. induction l. reflexivity.
    simpl. rewrite -> snoc_app. rewrite -> snoc_app. 
    rewrite -> fmap_app. rewrite IHl. reflexivity.
Qed. 
...
(* three nat doesn't work - because of universe inconsistenty;
   a violation of the stratification of languages.

   yet this is what we need. maybe one should implement nats at another level?.
*)

Definition const (X: Type) (Y: Type): X -> Y -> X :=
    fun x _ => x.

Definition exp (n m : nat) : nat :=
    (* generating of list of the right length *)
    let unit_list := m (list unit) (app [tt]) [] in
    (* replacing unit with curried multiplication *)
    let fun_list := map (const (nat -> nat) unit (mult n)) unit_list in
    (* folding of the list with 'one' as a seed  *)
    fold (fun  (f: nat -> nat) (n: nat) => f n) fun_list one.
